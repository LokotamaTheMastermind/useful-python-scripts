    ...
    while True:
        def normal_search(site_name, site_domain, query):
            if query != "":
                ...
                import platform
                op = platform.system()
                custom_website_search = f"https://www.{site_name}.{site_domain}/search?q={query}/"  # Constructed search uri
        ...
        def custom_search(site_name, site_domain, query_structure, query):
            if query != "":
                import webbrowser
                import time
                custom_website_search = f"https://www.{site_name}.{site_domain}/{query_structure}={query}"  # Constructed search query
                print(f"\nYour search query has been saved and processed opening in {site_name}")
                time.sleep(3)
                webbrowser.open(custom_website_search)

        question = input("\nDoes the site use the normal structure of search queries ['www.example.com/search?q=your_query']$ |-> ")
